...
sites <- read.csv("sites_2022.csv")

# import relocation data
elk.data <- read.csv("Relocations_2022.csv")
...
used.lsm.df.1 <- used.lsm.df[ ,c(2:22)]
...
for (z in unique(avail.spdf.proj$Animal)[6:35]) {
  ...
  # make sure that plot_id is numeric
  avail.lsm$plot_id <- as.numeric(avail.lsm$plot_id)
  ...
  nonexistent5ai.250 <- function(x) {
    
    if (5 %notin% avail.lsm$class[avail.lsm$plot_id == x & avail.lsm$buffer == 250 & avail.lsm$metric == "ai"])
      c(250, 1, "class", 5, NA, "ai", 0, paste0(x), NA)
  }
  
  nonexistent5ai.500 <- function(x) {
    
    if (5 %notin% avail.lsm$class[avail.lsm$plot_id == x & avail.lsm$buffer == 500 & avail.lsm$metric == "ai"])
      c(500, 1, "class", 5, NA, "ai", 0, paste0(x), NA)
  }
  
  nonexistent5ai.1000 <- function(x) {
    
    if (5 %notin% avail.lsm$class[avail.lsm$plot_id == x & avail.lsm$buffer == 1000 & avail.lsm$metric == "ai"])
      c(1000, 1, "class", 5, NA, "ai", 0, paste0(x), NA)
  }
  
  # Apply functions over all plot IDs
  avail.lsm5ai.250LIST <- lapply(min(avail.lsm$plot_id):max(avail.lsm$plot_id), nonexistent5ai.250)
  avail.lsm5ai.500LIST <- lapply(min(avail.lsm$plot_id):max(avail.lsm$plot_id), nonexistent5ai.500)
  avail.lsm5ai.1000LIST <- lapply(min(avail.lsm$plot_id):max(avail.lsm$plot_id), nonexistent5ai.1000)
  
  # Bind all vectors together from list
  avail.lsm5ai.250 <- plyr::ldply(avail.lsm5ai.250LIST, rbind)
  avail.lsm5ai.500 <- plyr::ldply(avail.lsm5ai.500LIST, rbind)
  avail.lsm5ai.1000 <- plyr::ldply(avail.lsm5ai.1000LIST, rbind)
  
  # bind together dataframes for all plot_id:buffer combinations that lack 5
  avail.lsm5ai <- rbind(avail.lsm5ai.250, avail.lsm5ai.500, avail.lsm5ai.1000)
  
  # bind all new data frames together
  avail.lsm.extra <- rbind(avail.lsm4, avail.lsm5, avail.lsm6, avail.lsm5ai)
  ...
  for (y in 1:length(unique(avail.lsm$plot_id))) {
    ...
    # buffer - 250
    # ai.mf
    avail.lsm.df[row.index, "ai.mf.250"] <- avail.lsm$value[avail.lsm$buffer == 250 &
                                                              avail.lsm$class == 5 &
                                                              avail.lsm$metric == "ai" &
                                                              avail.lsm$plot_id == row.index]
    # pr
    avail.lsm.df[row.index, "pr.250"] <- avail.lsm$value[avail.lsm$buffer == 250 &
                                                           avail.lsm$metric == "pr" &
                                                           avail.lsm$plot_id == row.index]
    
    # buffer - 500
    # ai.mf
    avail.lsm.df[row.index, "ai.mf.500"] <- avail.lsm$value[avail.lsm$buffer == 500 &
                                                            avail.lsm$class == 5 &
                                                            avail.lsm$metric == "ai" &
                                                            avail.lsm$plot_id == row.index]
    # pr
    avail.lsm.df[row.index, "pr.500"] <- avail.lsm$value[avail.lsm$buffer == 500 &
                                                         avail.lsm$metric == "pr" &
                                                         avail.lsm$plot_id == row.index]
    
    # buffer - 1000
    # ai.mf
    avail.lsm.df[row.index, "ai.mf.1000"] <- avail.lsm$value[avail.lsm$buffer == 1000 &
                                                            avail.lsm$class == 5 &
                                                            avail.lsm$metric == "ai" &
                                                            avail.lsm$plot_id == row.index]
    # pr
    avail.lsm.df[row.index, "pr.1000"] <- avail.lsm$value[avail.lsm$buffer == 1000 &
                                                         avail.lsm$metric == "pr" &
                                                         avail.lsm$plot_id == row.index]
  }
...
# raster directory 2
raster.dir.2 <- "D:/Elk Project/Raster processing"
...
ED.250 <- raster(paste0(raster.dir.2, "/", "edge_250.tif"))
ED.500 <- raster(paste0(raster.dir.2, "/", "edge_500.tif"))
ED.1000 <- raster(paste0(raster.dir.2, "/", "edge_1000.tif"))
...
used.data.all.3 <- cbind(used.spdf.proj@data, used.data.all.2[ ,c(17:58)])

# change NAs to zero and divide by 10 to get true ED values
used.data.all.3[ ,c("ED.250", "ED.500", "ED.1000")][is.na(used.data.all.3[ ,c("ED.250", "ED.500", "ED.1000")])] <- 0

used.data.all.3[ ,c("ED.250", "ED.500", "ED.1000")] <- used.data.all.3[,c("ED.250", "ED.500", "ED.1000")] / 10
...
# change NAs to zero and divide by 10 to get true ED values
avail.data.all.2[ ,c("ED.250", "ED.500", "ED.1000")][is.na(avail.data.all.2[ ,c("ED.250", "ED.500", "ED.1000")])] <- 0

avail.data.all.2[ ,c("ED.250", "ED.500", "ED.1000")] <- avail.data.all.2[,c("ED.250", "ED.500", "ED.1000")] / 10

# bind used and available together
avail.data.all.2 <- avail.data.all.2 %>% dplyr::select(-index)
...
# raster data
# raster directory
EVI.dir <- "D:/Elk project/EVI rasters/EVI 2022"

evi.509 <- raster(paste0(EVI.dir, "/", "5_9_2022.tif"))

evi.525 <- raster(paste0(EVI.dir, "/", "5_25_2022.tif"))

evi.610 <- raster(paste0(EVI.dir, "/", "6_10_2022.tif"))

evi.626 <- raster(paste0(EVI.dir, "/", "6_26_2022.tif"))

evi.712 <- raster(paste0(EVI.dir, "/", "7_12_2022.tif"))

# raster proj
EVI.proj <- evi.509@crs
...
for (x in unique(part.data.1$Animal)) {
  ...
  if (indiv.date > as.Date("2022-05-08") & indiv.date < as.Date("2022-05-25")) {
    
    use.raster <- evi.509
    
  } else {
    
    if (indiv.date > as.Date("2022-05-24") & indiv.date < as.Date("2022-06-10")) {
      
      use.raster <- evi.525
      
    } else {
      
      if (indiv.date > as.Date("2022-06-09") & indiv.date < as.Date("2022-06-26")) {
        
        use.raster <- evi.610
        
      } else {
        
        if (indiv.date > as.Date("2022-06-25") & indiv.date < as.Date("2022-07-12")) {
...
write.csv(part.data.2, "part_site_data_2022.csv")
